...
# Add counts > 0
count <- aggregate( Species ~ transectID, FUN = length, data = dat_det) 
...
## ---- 3.1. Variables (same as detection component from Mougeot et al. 2021 = National Survey, NS) ----
...
h_sc <- (h - h_mean) / h_sd # 2. Centered on the variable used in NS (squared mean and sd) 
...
# Prior info for coefficients of sigma (Mougeot et. al 2021)
...
polygon(c(dens_obs1$x, 0), c(dens_obs1$y, 0), col="darkcyan", border = "darkcyan") 
...
polygon(c(dens_obs2$x, 0), c(dens_obs2$y, 0), col="darkcyan", border = "darkcyan",lwd = 1.5) 
